...
const cases = [
  { name: 'prettify/realistic/10KB', input: () => makeRealisticHtml(10 * KB), iterations: 10, run: prettify },
  { name: 'prettify/realistic/100KB', input: () => makeRealisticHtml(100 * KB), iterations: 5, run: prettify },
  { name: 'prettify/realistic/1MB', input: () => makeRealisticHtml(MB), iterations: 2, run: prettify },
  { name: 'minify/realistic/100KB', input: () => makeRealisticHtml(100 * KB), iterations: 5, run: minify },
  ...
  { name: 'minify/text-run/32KB', input: () => `<div>${'x'.repeat(32 * KB)}</div>`, iterations: 2, run: minify },
  { name: 'isHtml/malformed/12', input: () => `<div ${'a '.repeat(12)}`, iterations: 5, run: isHtml },
  { name: 'isHtml/malformed/16', input: () => `<div ${'a '.repeat(16)}`, iterations: 5, run: isHtml },
  { name: 'isHtml/malformed/20', input: () => `<div ${'a '.repeat(20)}`, iterations: 3, run: isHtml },
  { name: 'prettify/content-wrap/100KB', input: () => makeRealisticHtml(100 * KB), iterations: 3, run: html => prettify(html, { content_wrap: 60 }) },
  { name: 'prettify/tag-wrap/100KB', input: () => makeRealisticHtml(100 * KB), iterations: 3, run: html => prettify(html, { tag_wrap: 40 }) },
  { name: 'prettify/deep/100', input: () => makeDeepHtml(100), iterations: 5, run: prettify },
  { name: 'prettify/deep/300', input: () => makeDeepHtml(300), iterations: 3, run: prettify },
  { name: 'prettify/ignored/100', input: () => makeIgnoredHtml(100), iterations: 3, run: html => prettify(html, { ignore: ['pre'] }) },
  { name: 'prettify/ignored/500', input: () => makeIgnoredHtml(500), iterations: 2, run: html => prettify(html, { ignore: ['pre'] }) },
  { name: 'prettify/ignored/1000', input: () => makeIgnoredHtml(1000), iterations: 1, run: html => prettify(html, { ignore: ['pre'] }) },
]
